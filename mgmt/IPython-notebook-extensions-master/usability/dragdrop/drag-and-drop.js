            ...
            if(event.stopPropagation) {event.stopPropagation();}
                if (event.dataTransfer.items != undefined) { 
                    ...
                    if (files.length == 0) {
                         ...
                         *   data - image is a base64 blob
                         */
                        /*console.log("FIREFOX",y"type:",url," name:", filename," path:", IPython.notebook.notebook_path," url:", url);*/
                        send_to_server(filename, IPython.notebook.notebook_path, data);                        
                        event.preventDefault();
                        return;
                        }
                    ...
                    for (var i=0; i < files.length; i++) {
                        ...
                        if (blob.type.indexOf('image/') !== -1) {
                            ...
                            var reader = new FileReader();
                                reader.onload = ( function(evt) {
                                    /*console.log("LOCAL FILE SYSTEM","file"," name:", filename," path:", IPython.notebook.notebook_path," url:", url);*/
                                    send_to_server(filename, IPython.notebook.notebook_path, evt.target.result);
                                    event.preventDefault();
                                    } );
                            reader.readAsDataURL(blob);
